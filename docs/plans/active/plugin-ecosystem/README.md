...
### M4e. Freeze, migrate, flag flip, site docs — proposed, td-6c49c5 (protocol and host), td-944274 (config and flags), td-c9de28 (recall repo) (recall), td-ade3a3 (site docs)

Decided 2026-09-03 from an audit of the freeze surface. Four lanes, then the plan set moves to `docs/plans/implemented/`.

**Freeze — td-6c49c5.** `pluginhost.Protocol` becomes `sidecar.plugin/v1`; the host sends it and validates the response identifier strictly, as it does today, so a plugin answering the draft identifier is a protocol failure and the reference's rule stays literally true. Tolerance lives on the plugin side: recall accepts both `sidecar.plugin/v1` and `sidecar.plugin/v1-draft` on a request and answers with whichever it was asked, so a Sidecar older than this release keeps working with a newer recall. The canonical JSON, the fixture provider's own constant, the two CLI tests that spell the literal, the reference (whose "Draft status" section becomes "Frozen"), the guide, the example plugin, and the generated CLI reference all change in the same commit. Of the six pending revisions, two ship before the freeze because they make the contract correct rather than richer: `params.filters{}` on `get`, sent exactly as the list that produced the row sent them (recall's expansion otherwise runs under the wrong profile and can be denied), and the 24-character render bound on `status.label`, stated as the plugin protocol's own bound beside the frozen `resource.MaxStatusLabelChars`. Two are host rendering rules with no wire change and ship in the same lane: the narrow reflow puts rank and primary on line one and folds the rest into line two as the mockup drew it, and an empty detail box in a Tab placement shows the plugin's next collection rather than help text. Two leave the table as decisions: the excerpt kind is a v2 note, because it changes the cell type and recall already degrades to plain text; and `asOf` is out of scope for v1, because a plugin that needs it declares a text filter and nothing in the tree sends it.
