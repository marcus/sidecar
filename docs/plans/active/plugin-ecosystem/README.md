...
**Status:** M1 through M3 merged to `main` on 2026-09-03 (merge ce052436), behind the `plugin_protocol` flag; the recall reference plugin (td-26c2b4, revised by td-786e42) is merged in its own repository; M4 is split: M4a (pointer and focus parity in the shared browser), M4b (scope filters, coverage, outcome honesty) and M4c (authoring guide and protocol reference) are implemented in [browser-parity-and-scope.md](browser-parity-and-scope.md), and M4d (usability pass) is implemented; M4e (freeze, migration, docs site, flag flip) is next. Implemented: M1 (descriptor and generalized global host, td-01b62b), M2a (the protocol host half — `internal/pluginhost`, `plugins.external`, the fixture and conformance suite, and the `sidecar plugin` CLI — td-a6d276), M2b (the shared browser `internal/pluginbrowser` and the protocol global tab, td-0d3539) and M3 (collection and row tabs in the Resource leaf on both surfaces, the `resources` live-refresh binding, `sidecar open --plugin`, `sidecar plugin changed`, and the layout spec's `collection`/`query`, td-44fe20). M0's mockups are in [mockups/](mockups/); four of the protocol revisions they surfaced were applied in M4b and the rest are still pending. Decisions settled with the maintainer on 2026-09-02. **Tracking:** td-f9f007.
...
### M4d. Usability pass — implemented, td-e8cceb td-21aafa (and td-c6904c) td-2b8f79 td-45898b

One shared query field with real editing keys and a clear control, a browser whose detail follows the cursor with superseded `get` calls cancelled, one selector control promoted from the Create Workspace modal into `internal/modal` and used by every View modal, and selectable text in the browser detail, issue, resource and diff panes. Details in [browser-parity-and-scope.md](browser-parity-and-scope.md#m4d-usability-pass-query-fields-detail-follows-the-cursor-selectors-selectable-text).

### M4e. Freeze, migrate, flag flip, site docs — freeze implemented (td-6c49c5); td-944274 (config and flags), td-c9de28 (recall repo), td-ade3a3 (site docs) proposed

Decided 2026-09-03 from an audit of the freeze surface. Four lanes, then the plan set moves to `docs/plans/implemented/`.

**Freeze — td-6c49c5, implemented.** `pluginhost.Protocol` is `sidecar.plugin/v1`. The host sends it and validates the response identifier strictly, with no alias, so a plugin answering the pre-freeze draft identifier is a protocol failure and the reference's rule is literally true rather than aspirational. Tolerance lives on the plugin side: a plugin that accepts either identifier on a request and answers with whichever it was asked works with a Sidecar from before the freeze and with every one after it, which is what recall does. The reference's "Draft status" section is now "Frozen: what that promises" and states that rule; the pre-freeze identifier is named in its History so an author can still find it. The canonical JSON, the fixture's own constant, the CLI test literals, the guide, the example plugin and the generated CLI reference all carry the frozen identifier, and the fixture keeps a `-mode=draft-answer` so the mismatch is a test rather than a claim.

Four of the six revisions the M0 mockup surfaced landed with it, and two closed as decisions; all six are recorded in [the settled table](#protocol-revisions-from-the-m0-recall-mockup--all-settled). `get.params.filters` carries the applied set of the list that produced the row, through every journey that reaches a `get`: the browser's detail box, the row tab Enter opens in a pane, a restored row tab, `sidecar open --plugin ID --collection C ROW --filter id=value`, a layout spec row, and `sidecar plugin check --get` / `plugin call get`. `status.label` has a stated 24-character render bound. The narrow reflow keeps the columns declared before the primary on line one, and an empty detail box in a `Tab` placement shows the plugin's next collection; both are host rules with no wire change, recorded in [host.md](host.md).

Six things came out differently from the paragraph above as it was first written, and the code is the authority:

1. **Line two's order is the mockup's, not the pending row's.** The pending row listed "status label, the remaining short columns, and the secondary text"; `mockups/recall-studio.narrow-pane.txt` draws `notes · exact · › people are tiered…` — the short column, then the status label, then the secondary — and the drawn artifact won.
2. **The rule is "the columns declared before the primary", not "rank".** The host does not know what a plugin calls its rank, and the plugin already said which column names the row. Declaration order is the whole rule, which is what makes it a host rule rather than a special case for one plugin.
3. **`Provider.Get` grew the validated `Collection` parameter**, exactly as `List` has one. Filters are narrowed at the process boundary, so "a key the collection did not declare never reaches the plugin" is a property of the host rather than a promise each caller keeps. `Manager.Get`'s cache key includes the applied set, because the same row under two scopes is two documents.
4. **`sidecar open` used to refuse a row and a filter together** as competing requests. They are not: the filters are the scope the row was found under. The refusal is gone, and the `--query` refusal beside it stays.
5. **A `Tab` placement now spends one extra `list` per session**, reading the next collection to fill the empty detail box. It is bounded to once, skipped while a document is open, and never run for a `search: required` collection.
6. **The remote content-read twin still carries no filters.** `sidecar content read --kind resource --operation collection|item` has no `filters` on the wire, so a Resource pane bound to a remote host lists and expands under the plugin's declared defaults there. This was already true of `list` before this lane and is unchanged by it; closing it is a wire change to `contentservice.CollectionParams` and its remote caller, and belongs with whatever next needs remote plugin panes to be scope-aware.

**Config and flags — td-944274.** `terminalResources.providers` becomes the read-only alias decision 8 promised: still read, still dispatched on `sidecar.terminal-resource/v1`, no longer written by `Save` (the raw section is preserved untouched), and `sidecar plugin list` keeps its `source` column so the answer is verifiable. Rewriting entries into `plugins.external` waits for the release that removes the alias, because a migrated entry would be asked the new identifier; that removal is filed as a follow-up with the rule that a moved entry records its protocol. `tasks_plugin` and `notes_plugin` become read-only aliases the same way: the Flags page rows read the descriptor (td-a360ff item 1), the config documentation names `plugins.<id>.enabled`, and the remaining flag readers move to the descriptor. Two saver and validation defects are fixed first because the migration is when they bite: `Save` merges unknown subkeys under `plugins` instead of dropping them (td-58ed37 item 1), `validatePluginInstances` refuses an external ID that collides with an embedded descriptor (item 3), and `sidecar plugin add` refuses the same collision (item 2). `plugin_protocol` and `terminal_resource_providers` both default on, with the settings rows still marked as needing a restart; the describe pass already waits for the first ready frame, and a `SIDECAR_STARTUP_TRACE` run on a config with a real plugin proves `first ready frame` lands before the describe.

**Recall — td-c9de28 (recall repo).** Accept both identifiers and echo the one asked; read `params.filters` on `get` and resolve the profile from it as `list` does; changelog and README name `sidecar.plugin/v1`.

**Site docs — td-ade3a3.** `website/docs/plugins.md` replaces `terminal-resources.md`, which documents a config shape that never existed; it moves out of Windowing & Durability into a new "Extending Sidecar" category, keeps the old URL as a redirect, links the frozen reference, the authoring guide and the runnable example by their GitHub URLs, keeps a short terminal-resource-providers section pointing at `terminal-links`, and `cli-reference.md` gains the `sidecar plugin` block it has never had.

**Then.** DEX and ongoing subcommands follow against the frozen contract in their own repositories; anything they cannot express is a v2 note. This plan set moves whole to `docs/plans/implemented/plugin-ecosystem/` (the first directory there, deliberately: a six-file set with mockups and proof captures does not flatten), every status header reads implemented, and the eleven inbound links plus the two sibling-relative outbound ones are fixed; the proof captures and the mock tab title that contain the old path as screen text are left alone.

**Evidence.** Conformance suite green on the frozen identifier with the fixture answering `v1`; recall proven against the frozen host through an isolated `sidecar plugin check` and a `get` under `profile=personal` that is no longer denied; a config carrying `terminalResources.providers` loaded, saved, and byte-identical in that section afterwards; a config with an unknown `plugins.foo` subkey surviving a save; the startup trace ordering; the site building with `onBrokenLinks: throw` and the redirect resolving; `grep -r plans/active/plugin-ecosystem` after the move matching only proof captures and the mock title.
...
## Protocol revisions from the M0 recall mockup — all settled

Writing recall's screens against its real `--help` surfaced six facts the draft could not carry. Each was a proposed revision to [the protocol reference](../../../reference/plugin-protocol.md); none is pending, and the table they lived in is gone with the freeze.

M4b applied four — the `failed` outcome, `page.omitted`, `filters[]` with `list.params.filters`, and `page.coverage[]` — plus the rule that `outcome` describes only the row set. M4e-a settled the remaining six:

| Gap | What it became |
| --- | --- |
| `get` carried no filters, so a row found under a filter-chosen scope expanded under the plugin's default scope and could be denied | **Applied.** `get.params.filters`, sent exactly as the `list` that produced the row sent it and narrowed against the declaration at the same process boundary. The host's get cache is keyed by the applied set. |
| `status.label` length was unbounded but the host has to fit it in a reserved column | **Applied.** A 24-character *render* bound in the reference's Limits table, as `pluginhost.StatusLabelRenderChars`. The frozen 64-character wire bound in `resource.MaxStatusLabelChars` is untouched; the reference says plainly that the two are different things. |
| The narrow reflow rule named only the secondary column | **Applied**, as a host rule with no wire change: the columns declared *before* the primary stay with it on line one, and the remaining short columns, the status label and the secondary fold into an indented line two. |
| An empty detail box in a `Tab` placement | **Applied**, as a host rule with no wire change: it shows the plugin's next collection — listed once, without a query, unless its search is required — so `abstained` is verifiable in place. A plugin with one collection keeps the help line. |
| Excerpts carry a kind (matched span, record opening, unmarked) that the mockup fakes with `›`/`~` prefixes the host cannot explain | **Decided: a v2 note.** It changes the cell type — a per-cell `{text, mark}` shape and a host-drawn legend — rather than adding a field, and recall already degrades to plain text with nothing lost but the mark. A change of that size is what a new protocol identifier is for. |
| `--as-of` changes what every row means and must survive refresh, but `list.params` has only `query`, `view`, `sort`, `filters`, `cursor` | **Decided: out of scope for v1.** A plugin that needs it declares a text filter, which is persisted with the tab and sent on every `list` and now on every `get`. Nothing in the tree sends an `asOf`, so the field would have been a shape with no caller. |
...
- 2026-09-03: M4e-a implemented (td-6c49c5). The protocol froze as `sidecar.plugin/v1` with the host strict and no alias; `get` carries the filters its list was run with; `status.label` has a stated 24-character render bound; the narrow reflow and the empty `Tab` detail box are stated host rules. The pending-revisions table is gone: four rows were applied, and the excerpt kind and `asOf` closed as decisions. Six deviations from the plan text are recorded in the M4e section above.
