...
The limitation is worth stating plainly: **binding is the only event recorded today.** Creating a managed shell in a project does not mark it active, and neither does an agent working in one — nothing watches terminal output, and nothing should be read into the column that it does not say. The set can widen later without the word changing meaning, because "the latest recorded Sidecar event" is already what it means; managed shell creation is the obvious next one to add, and it is one call site rather than a redefinition.
