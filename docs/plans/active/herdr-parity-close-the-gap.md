...
**One thing left undone.** The detail pane still truncates rather than scrolls, so a pane shorter than the page clips its bottom. With seventeen providers at 80x30 that is the detail box and the closing note. This is the surface's pre-existing behaviour, only the Feature Flags page scrolls, and Slice 6 does not ask for scrolling; generalising `scrollFlagsPage`, or windowing just the row block, is the fix when someone wants it.
