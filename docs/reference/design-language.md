...
- **A bar that is a mode rather than a filter keeps its own `Esc`, and leaves in one press.** The in-file searches (docview, the file browser's preview, a note's preview) and the terminal search bars are opened over something the reader is looking at, own every key in the pane while they are up, and are left rather than narrowed; two presses to get out of one is a worse gesture, not a more consistent one. Those bars answer `Esc` themselves and never offer it to the field. Every bar that filters a list beside it follows the field's rule above. Which kind a bar is, is the surface's to say, and it says so where the key is answered.
...
- **A key that hands off does not type itself.** `k` means "up a row" and, on the first row, "back to the query", so a user holding it to scroll a list up arrives at the bar with the rest of the repeat burst still coming — as text, and as a query nobody typed. The surface that owns the hand-off guards the arriving key for one key-repeat window (250 ms, pushed forward by each repeat): while the same key keeps arriving it is inert, and any other key or a longer gap ends the guard. The guard belongs to the surface, not to `queryfield`, because it is about how focus arrived; a bar reached by `/` or by a click has nothing to guard against.
...
Every pane that draws text a reader might quote lets them drag over it and copy it. There is one engine for this — `internal/textselect` — and a pane joins it by writing an adapter, never by implementing a gesture:

- **The pane implements `textselect.Source`**: where its content is drawn (`ContentRect`, in the coordinate space its mouse events arrive in), what its visual rows say after wrapping (`Line`, `LineCount`), and how far it is scrolled (`Scroll`). Rows are *visual*, post-wrap and post-tab-expansion, so what the reader sees is what they copy, and the row indices are the pane's own coordinate space, so a selection stays on the text it was made over while the pane scrolls under it. A pane whose rows are painted one window at a time by something else — the diff pane, whose patch body is drawn by `gitstatus`' renderers with gutters the raw patch does not carry — makes the frame its coordinate space instead, and says so by dropping the selection when the frame changes.
- **The highlight is painted through `Surface.DecorateRow` at slice time**, onto the row about to be drawn and never into anything the pane caches. A selection belongs to one frame; a cached row carrying a highlight is a highlight that outlives what it described.
- **The copy goes through `clip.Copy`**, which writes the system clipboard and the terminal's own over OSC 52, so a copy works over SSH and inside tmux. Hosts never phrase the notice themselves: `Keys.CopySelectionCmd` produces it, and the host only chooses which toast type carries it.
- **The host routes, and decides nothing.** It says where the pane was drawn (`SetOrigin`), binds the chords from config (`SetSelection`), and forwards the press, the drag, the release, the lost release and the chords through `textselect.Pane`. One selection at a time is the surface's rule: starting one anywhere drops the one before it, and a modal opening, focus leaving, or the document changing abandons it.
- **A press arms a selection and still does what it did before.** The click that focuses a pane, opens a row or navigates to a subtask happens on the press as it always did; the gesture only decides what the *motion after it* means. `Result.ClickThrough` is how a release without motion says so. A pane whose cells are its own targets — an issue card's parent and subtask rows, a diff pane's file list — leaves those clicks alone and offers the rest of its body as text.
- **Holding shift or option hands the drag to the terminal emulator instead.** That is the emulator's own bypass of mouse reporting, not something Sidecar implements: it is how a reader reaches their emulator's native selection and quick-copy over any pane, including one Sidecar does not make selectable. The help modal says so, because it is the answer to "why can I not select that".
...
**A list-shaped selector is a bordered control, and its border follows focus.** The rows and their `↑ more above` / `↓ more below` markers sit inside a rounded border coloured by exactly the ladder `modal.Input` uses: `BorderNormal` idle, `styles.Primary` focused, `TextMuted` under the pointer. The segmented shape's `[ ]` frame carries the same ladder, so both shapes answer the same question the same way. The selected row keeps its `Primary` fill whether or not the control has focus — the fill says which choice is active, the border says where the keyboard is — which is what lets a modal hold three selectors at once and still be read. Without the border it could not: the View modal's Scope, Source and Type controls each painted a gold row and nothing said which one Tab was on. The border is chrome, not a choice: a click on a border cell focuses the control and selects nothing.

**A modal sizes to the widest control it holds.** A modal's width is the larger of its own default and the natural width of its widest control, capped to the frame it floats over. `modal.WidthForSections` computes it from the sections a host is about to add, budgeting for the box's border, its padding, and the column a scrollbar takes once the body scrolls. A segmented control's natural width is its whole label row, the two columns `styles.Button` pads each segment with included, because the alternative to fitting it is truncating it. Where even the capped width cannot hold the segments, the shape rule's width floor drops the control to the list, which always fits. A modal held at a fixed width with a control too wide for it produces the failure this rule exists to prevent: `[ Relevance | Source | Up… ]`.
