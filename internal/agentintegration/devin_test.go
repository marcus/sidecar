...
func devinFixture(t *testing.T, opts ...func(*Env)) (Service, Env, sessionHookPaths) {
	t.Helper()
	home := t.TempDir()
	env := Env{
		Home:       home,
	...
	return Service{Env: env, Adapters: DefaultAdapters()}, env, devinIntegration().pathsFor(env)
}
...
// to proceed without.
func devinSetUp(t *testing.T, paths sessionHookPaths) {
	t.Helper()
...
func TestDevinInstallsUnderExactlyTheEventsUpstreamNames(t *testing.T) {
	...
	events := devinIntegration().spec.eventNames()
	...
	if devinIntegration().spec.matcher != nil {
		t.Fatal("the Devin group carries a matcher; install_devin writes none")
	}
	if strings.Contains(string(devinIntegration().canonicalFile()), `"matcher"`) {
...
func TestDevinInstallWritesEveryEventAndPreservesTheRest(t *testing.T) {
	...
	devinSetUp(t, paths)
	writeFileForTest(t, paths.File, `{"theme_mode":"dark","hooks":{}}`)
	...
	for _, event := range devinIntegration().spec.eventNames() {
...
func TestDevinInstallIsIdempotentAcrossEveryEvent(t *testing.T) {
	...
	applyTo(t, svc, DevinProvider, ActionInstall)
	first := readFileForTest(t, paths.File)
	...
	if got := readFileForTest(t, paths.File); got != first {
...
func TestDevinRepairsAPartialInstall(t *testing.T) {
	...
	top := mustParseAny(t, readFileForTest(t, paths.File)).(map[string]any)
...
func TestDevinNeverAdoptsAnEntryThatMerelyLooksLikeSidecars(t *testing.T) {
  ...
  "hooks": {
	...
	applyTo(t, svc, DevinProvider, ActionUninstall)
	if got := mustParseAny(t, readFileForTest(t, paths.File)); !reflect.DeepEqual(got, mustParseAny(t, before)) {
		t.Fatalf("uninstall did not restore the user's file:\nbefore %s\nafter  %s", before, readFileForTest(t, paths.File))
	}
}
...
func TestDevinFollowsXDGConfigHome(t *testing.T) {
	...
	unset := devinIntegration().pathsFor(Env{Home: home})
	if want := filepath.Join(home, ".config", "devin", "config.json"); unset.File != want {
		t.Fatalf("with no XDG_CONFIG_HOME the settings path is %q, want %q", unset.File, want)
	}
	set := devinIntegration().pathsFor(Env{Home: home, ConfigHome: elsewhere})
	if want := filepath.Join(elsewhere, "devin", "config.json"); set.File != want {
		t.Fatalf("with XDG_CONFIG_HOME the settings path is %q, want %q", set.File, want)
	}
	tilde := devinIntegration().pathsFor(Env{Home: home, ConfigHome: "~/somewhere"})
	if want := filepath.Join(home, "somewhere", "devin", "config.json"); tilde.File != want {
		t.Fatalf("a tilde in XDG_CONFIG_HOME resolved to %q, want %q", tilde.File, want)
	}
	blank := devinIntegration().pathsFor(Env{Home: home, ConfigHome: "   "})
	if want := filepath.Join(home, ".config", "devin", "config.json"); blank.File != want {
		t.Fatalf("a whitespace-only XDG_CONFIG_HOME resolved to %q, want the default %q", blank.File, want)
	}
}
...
func TestDevinBacksUpTheUsersFileBeforeRewritingIt(t *testing.T) {
	...
	writeFileForTest(t, paths.File, original)
