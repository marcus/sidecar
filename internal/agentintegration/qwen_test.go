...
func qwenFixture(t *testing.T, opts ...func(*Env)) (Service, Env, sessionHookPaths) {
	t.Helper()
	home := t.TempDir()
	env := Env{
		Home: home,
	...
	return Service{Env: env, Adapters: DefaultAdapters()}, env, qwenIntegration().pathsFor(env)
}
...
func TestQwenInstallsUnderExactlyTheEventUpstreamNames(t *testing.T) {
	...
	events := qwenIntegration().spec.eventNames()
	...
	if qwenIntegration().spec.matcher == nil || *qwenIntegration().spec.matcher != "*" {
		t.Fatalf("the Qwen group's matcher is %v, want \"*\"", qwenIntegration().spec.matcher)
	}
}
...
func TestQwenCountsItsTimeoutInMilliseconds(t *testing.T) {
	...
	// Asserted against the shipped bytes rather than against a field, because
	// the bytes are what the provider reads.
	if !strings.Contains(string(qwenIntegration().canonicalFile()), `"timeout": 10000`) {
		t.Fatalf("the canonical Qwen entry does not carry a millisecond timeout:\n%s", qwenIntegration().canonicalFile())
	}
	...
	for _, i := range []sessionHookIntegration{devinIntegration(), droidIntegration(), qoderCLIIntegration()} {
		want := `"timeout": ` + strconv.Itoa(hookTimeoutSec)
		if !strings.Contains(string(i.canonicalFile()), want) {
			t.Errorf("%s does not carry %s; it counts in seconds:\n%s", i.provider, want, i.canonicalFile())
		}
	}
}

func TestQwenBundledEntryMatchesTheRegistry(t *testing.T) {
	asset := NewQwenAdapter().asset()
...
func TestQwenFollowsQwenHome(t *testing.T) {
	...
	unset := qwenIntegration().pathsFor(Env{Home: home})
	if want := filepath.Join(home, ".qwen", "settings.json"); unset.File != want {
		t.Fatalf("with no override the settings path is %q, want %q", unset.File, want)
	}
	set := qwenIntegration().pathsFor(Env{Home: home, QwenHome: elsewhere})
	if want := filepath.Join(elsewhere, "settings.json"); set.File != want {
		t.Fatalf("with QWEN_HOME the settings path is %q, want %q", set.File, want)
	}
	tilde := qwenIntegration().pathsFor(Env{Home: home, QwenHome: "~/somewhere"})
	if want := filepath.Join(home, "somewhere", "settings.json"); tilde.File != want {
		t.Fatalf("a tilde in QWEN_HOME resolved to %q, want %q", tilde.File, want)
	}
	blank := qwenIntegration().pathsFor(Env{Home: home, QwenHome: "   "})
	if want := filepath.Join(home, ".qwen", "settings.json"); blank.File != want {
		t.Fatalf("a whitespace-only QWEN_HOME resolved to %q, want the default %q", blank.File, want)
	}
	...
	if got := qoderCLIIntegration().pathsFor(env).Dir; got == elsewhere {
		t.Fatal("QWEN_HOME moved the Qoder directory")
	}
	if got := qwenIntegration().pathsFor(env).Dir; got != elsewhere {
		t.Fatalf("the Qwen directory is %q, want %q", got, elsewhere)
	}
	if got := droidIntegration().pathsFor(env).Dir; got != filepath.Join(home, ".factory") {
...
func TestQwenInstallWritesTheEntryAndPreservesTheRest(t *testing.T) {
	...
	writeFileForTest(t, paths.File, `{"theme":"Default","selectedAuthType":"openai"}`)
...
func TestQwenNeverAdoptsAnEntryThatMerelyLooksLikeSidecars(t *testing.T) {
  ...
  "hooks": {
	...
	before := readFileForTest(t, paths.File)
	...
	applyTo(t, svc, QwenProvider, ActionInstall)
	applyTo(t, svc, QwenProvider, ActionUninstall)
	if got := readFileForTest(t, paths.File); mustParseAny(t, got) == nil || !strings.Contains(got, "echo sidecar") {
