...
const (
	...
	// DevinAssetSchema is the plan/marker schema the asset declares.
	DevinAssetSchema = 1
)
...
// devinCanonicalEntry is the exact handler version 1 ships.
func devinCanonicalEntry() json.RawMessage {
	return marshalJSONObject([]jsonMember{
		{key: "type", val: json.RawMessage(`"command"`)},
		{key: "command", val: mustJSONString(reportSessionCommand(DevinProvider))},
		{key: "timeout", val: json.RawMessage(strconv.Itoa(hookTimeoutSec))},
	})
}

// devinCanonicalGroup is the group the entry is installed in.
//
// It carries no matcher, which is what Herdr's install_devin writes:
...
func devinCanonicalGroup() json.RawMessage {
	return marshalJSONObject([]jsonMember{
		{key: "hooks", val: marshalJSONArray([]json.RawMessage{devinCanonicalEntry()})},
	})
}

func devinEntrySpec() hookEntrySpec {
	return hookEntrySpec{
		events: devinHookEvents,
		canonical: []versionedEntry{
			{version: DevinAssetVersion, entry: devinCanonicalEntry()},
		},
	}
}

// DevinConfigDir resolves the directory the Devin CLI keeps its configuration
// in: $XDG_CONFIG_HOME/devin when that variable names one, ~/.config/devin
// otherwise. It is the one provider in this group whose directory hangs off
// $XDG_CONFIG_HOME rather than off $HOME directly, which is Herdr's devin_dir.
func DevinConfigDir(env Env) string {
	if base := overrideDir(env.Home, env.ConfigHome); base != "" {
		...
		return filepath.Join(base, DevinProvider)
	}
	if strings.TrimSpace(env.Home) == "" {
		return ""
	}
	return filepath.Join(env.Home, ".config", DevinProvider)
}

// devinIntegration is the Devin integration as data.
func devinIntegration() sessionHookIntegration {
	return sessionHookIntegration{
		...
		source:       DevinSource,
		assetVersion: DevinAssetVersion,
		assetSchema:  DevinAssetSchema,
		fileName:     "config.json",
		dir:          DevinConfigDir,
		spec:         devinEntrySpec(),
		item:         devinCanonicalGroup,
...
type DevinAdapter struct{ sessionHookAdapter }

// NewDevinAdapter returns the adapter this build ships.
func NewDevinAdapter() DevinAdapter {
	return DevinAdapter{sessionHookAdapter{integration: devinIntegration()}}
}

// DevinPaths returns the paths the Devin adapter inspects and touches.
func DevinPaths(env Env) []string {
	return []string{devinIntegration().pathsFor(env).File}
}
