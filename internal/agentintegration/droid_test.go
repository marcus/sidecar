...
func droidFixture(t *testing.T, opts ...func(*Env)) (Service, Env, sessionHookPaths) {
	t.Helper()
	home := t.TempDir()
	env := Env{
		Home: home,
	...
	return Service{Env: env, Adapters: DefaultAdapters()}, env, droidIntegration().pathsFor(env)
}

func droidSetUp(t *testing.T, paths sessionHookPaths) {
	t.Helper()
...
func TestDroidInstallsUnderExactlyTheEventUpstreamNames(t *testing.T) {
	...
	events := droidIntegration().spec.eventNames()
	...
	if droidIntegration().spec.matcher != nil {
...
func TestDroidLivesUnderTheFactoryDirectoryWithNoOverride(t *testing.T) {
	home := t.TempDir()
	// None of the variables that move a neighbouring provider moves this one,
	// which is the assertion the absent override reduces to: Droid resolves its
	// directory from $HOME and from nothing else.
	for _, env := range []Env{
		{Home: home},
		{Home: home, ConfigHome: filepath.Join(t.TempDir(), "xdg")},
		{Home: home, QoderConfigDir: filepath.Join(t.TempDir(), "qoder")},
		{Home: home, QwenHome: filepath.Join(t.TempDir(), "qwen")},
	} {
		got := droidIntegration().pathsFor(env).File
...
func TestDroidInstallWritesOneEntryAndPreservesTheRest(t *testing.T) {
	...
	droidSetUp(t, paths)
	writeFileForTest(t, paths.File, `{"model":"claude-opus","hooksDisabled":false}`)
...
func TestDroidNeverAdoptsAnEntryThatMerelyLooksLikeSidecars(t *testing.T) {
  ...
  "hooks": {
	...
	applyTo(t, svc, DroidProvider, ActionInstall)
	groups := mustParseAny(t, readFileForTest(t, paths.File)).(map[string]any)["hooks"].(map[string]any)["SessionStart"].([]any)
	if len(groups) != 2 {
