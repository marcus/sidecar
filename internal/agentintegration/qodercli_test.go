...
func qoderCLIFixture(t *testing.T, opts ...func(*Env)) (Service, Env, sessionEntryPaths) {
	t.Helper()
	home := t.TempDir()
	env := Env{
		Home: home,
	...
	return Service{Env: env, Adapters: DefaultAdapters()}, env, qoderCLISpec().pathsFor(env)
}
...
func TestQoderCLIInstallsUnderExactlyTheEventUpstreamNames(t *testing.T) {
	...
	events := qoderCLISpec().eventNames()
	...
	if qoderCLISpec().matcher == nil || *qoderCLISpec().matcher != "*" {
		t.Fatalf("the Qoder group's matcher is %v, want \"*\"", qoderCLISpec().matcher)
	}
	if !strings.Contains(string(qoderCLISpec().content()), `"matcher": "*"`) {
		t.Fatalf("the canonical Qoder group does not render the matcher:\n%s", qoderCLISpec().content())
	}
}
...
func TestQoderCLIBundledEntryMatchesTheRegistry(t *testing.T) {
	asset := NewQoderCLIAdapter().settingsAsset()
...
func TestQoderCLIFollowsItsConfigDirOverride(t *testing.T) {
	...
	unset := qoderCLISpec().pathsFor(Env{Home: home})
	if want := filepath.Join(home, ".qoder", "settings.json"); unset.Settings != want {
		t.Fatalf("with no override the settings path is %q, want %q", unset.Settings, want)
	}
	set := qoderCLISpec().pathsFor(Env{Home: home, QoderConfigDir: elsewhere})
	if want := filepath.Join(elsewhere, "settings.json"); set.Settings != want {
		t.Fatalf("with QODER_CONFIG_DIR the settings path is %q, want %q", set.Settings, want)
	}
	tilde := qoderCLISpec().pathsFor(Env{Home: home, QoderConfigDir: "~/somewhere"})
	if want := filepath.Join(home, "somewhere", "settings.json"); tilde.Settings != want {
		t.Fatalf("a tilde in QODER_CONFIG_DIR resolved to %q, want %q", tilde.Settings, want)
	}
	blank := qoderCLISpec().pathsFor(Env{Home: home, QoderConfigDir: "   "})
	if want := filepath.Join(home, ".qoder", "settings.json"); blank.Settings != want {
		t.Fatalf("a whitespace-only QODER_CONFIG_DIR resolved to %q, want the default %q", blank.Settings, want)
	}
	...
	if got := qoderCLISpec().pathsFor(Env{Home: home, QoderConfigDir: elsewhere}).Dir; got != elsewhere {
...
func TestAChangedMatcherReadsAsDamageRatherThanAsCurrent(t *testing.T) {
	...
	edited := strings.Replace(readFileForTest(t, paths.Settings), `"matcher": "*"`, `"matcher": "startup"`, 1)
	if edited == readFileForTest(t, paths.Settings) {
		t.Fatal("the fixture did not change the matcher")
	}
	writeFileForTest(t, paths.Settings, edited)
