...
//
// It is a second function rather than four more rows in the table above because
// the property differs at the end: a session-identity entry spawns
...
// integration registered without being checked here is not possible.
func TestSessionIdentityEntriesSpawnArgvTheShippedCLIAccepts(t *testing.T) {
	providers := agentintegration.SessionEntryProviders()
	if len(providers) == 0 {
		t.Skip("no session-identity entry integrations are registered")
	}
	for _, provider := range providers {
		t.Run(provider, func(t *testing.T) {
			argvs := agentintegration.SessionEntryArgvCorpus(provider)
			if len(argvs) == 0 {
