...
//
// It reads through readHookPayload and hookPayload.sessionID rather than
// re-deriving the answer, so a change to either that broke a branch fails here
// against the fixture rather than passing against a copy of itself.
		...
		t.Run(row.name, func(t *testing.T) {
			...
			case "subagent":
				if strings.TrimSpace(payload.AgentID) == "" {
					t.Fatal("the fixture records a sub-agent payload but no agent_id survived decoding")
				}
			case "id":
				if strings.TrimSpace(payload.AgentID) != "" {
					t.Fatalf("agent_id %q would make this a sub-agent payload", payload.AgentID)
				}
			...
			case "path":
				...
				if got := strings.TrimSpace(payload.TranscriptPath); got != row.want {
					t.Fatalf("transcript path = %q, the fixture records %q", got, row.want)
				}
			case "none":
				if got := payload.sessionID(); got != "" {
					t.Fatalf("sessionID() = %q on a payload the fixture records as naming no session", got)
				}
				if got := strings.TrimSpace(payload.TranscriptPath); got != "" {
					t.Fatalf("transcript path = %q on a payload the fixture records as naming no session", got)
				}
			default:
